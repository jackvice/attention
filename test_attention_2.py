...
def process_sequence(
    ...
    """
    Process a sequence and save debug images if human detected in last frame.
    
    Returns:
        True if human detected in last frame, False otherwise
    """
    ...
    # Save debug images using write_debug_images
    from trajectory_utils import write_debug_images
    
    sequence_output_dir = os.path.join(output_dir, sequence_name)
    write_debug_images(
        rgb_frames=rgb_frames,
        mask_frames=mask_frames,
        prediction_heatmap=heatmap_np,
        epoch=1,  # Use 1 as a placeholder since this isn't training
        output_dir=sequence_output_dir,
        target_heatmap=None,  # No target for inference
        future_frame=rgb_frames[-1],  # Use last frame as "future"
        future_pedestrians=all_pedestrians[-1]  # Pedestrians in last frame
    )
...
def main():
    ...
    for sequence_dir in all_sequences:
        ...
        # Process sequence
        has_human = process_sequence(
            frame_paths, yolo_session, predict_fn, sequence_name, args.output_dir
        )
        
        if has_human:
            sequences_with_humans += 1
            print(f"  ✓ Found human in {sequence_name} - saved debug images")
