...
def attach_single_frame_memory(shm_name: str = SHM_NAME):
    """
    ...
    try:
        ...
        # Create it if environment hasn't started yet
        size = 8 + H*W*3
        shm = shared_memory.SharedMemory(create=True, size=size, name=shm_name)
        struct.pack_into('<d', shm.buf, 0, 0.0)  # Init timestamp
        return shm


def attach_single_frame_memory_old(shm_name: str = SHM_NAME) -> shared_memory.SharedMemory:
    """
    Attach to single-slot shared memory.
